...
import { DEFAULT_BUDGET_MS } from '../src/engine/deadline';
import { evaluateCatalog } from '../src/evaluate-offers';
...
// The budget matters to one test only, so it defaults here rather than being
// repeated at every call site.
const evaluate = (
  context: UserContext,
  catalog: readonly Offer[],
  budgetMs: number = DEFAULT_BUDGET_MS,
) => evaluateCatalog(context, catalog, budgetMs);

describe('evaluateOffers', () => {
  describe('wiring', () => {
    it('returns every baseline offer for an adult US resident', async () => {
      const result = await evaluate(buildUserContext(), baselineOffers);

      expect(idsOf(result.offers)).toEqual(['baseline-savings', 'baseline-credit-card']);
    });

    it('returns nothing for a user under 18', async () => {
      const result = await evaluate(buildUserContext({ user: { age: 17 } }), baselineOffers);
  ...
  describe('isolation', () => {
    it('never returns the Meridian exclusive for a user with no partner claim', async () => {
      const result = await evaluate(buildUserContext(), catalogWith(scoringService(800)));
    ...
    it('never returns the Meridian exclusive for a user holding another partner\'s claim', async () => {
      ...
      const result = await evaluate(context, catalogWith(scoringService(800)));

      expect(idsOf(result.offers)).not.toContain('meridian-savings');
    });

    it('never returns the Meridian exclusive when the credit score falls short', async () => {
      const result = await evaluate(buildMeridianMember(), catalogWith(scoringService(639)));
  ...
  describe('precedence', () => {
    it('gives a qualifying Meridian member the exclusive rate instead of the baseline savings offer', async () => {
      const result = await evaluate(buildMeridianMember(), catalogWith(scoringService(700)));
    ...
    it('adds the Northwind advance alongside the baseline offers rather than replacing one', async () => {
      const result = await evaluate(buildNorthwindMember(), catalogWith(scoringService(800)));
    ...
    it('supersedes on one product and adds on another for a member of both partners', async () => {
      ...
      const result = await evaluate(context, catalogWith(scoringService(700)));
  ...
  describe('degraded path', () => {
    it('still returns the baseline offers when the credit score service fails', async () => {
      const result = await evaluate(buildMeridianMember(), catalogWith(failingScoreService));
    ...
    it('withholds the Meridian exclusive when the credit score service never answers', async () => {
      const result = await evaluate(buildMeridianMember(), catalogWith(hangingScoreService), 20);
    ...
    it('still returns an earned Northwind advance when the credit score service fails', async () => {
      ...
      const result = await evaluate(context, catalogWith(failingScoreService));
